...
function convTokens(msgCount: number): Array<{ role: "user" | "assistant" | "system"; tokens: number }> {
...
async function seed() {
  ...
  const existingWs = await db.select({ id: workspaces.id }).from(workspaces).where(eq(workspaces.slug, "acme-inc")).limit(1);
  ...
  const existingUser = await db.select({ id: users.id }).from(users).where(eq(users.email, "demo@fichap.com")).limit(1);
  ...
  await db.insert(agents).values([
    { id: agentIds.hrMain, workspaceId, teamId: teamHrId, name: "Sofia HR", role: "HR Generalist", systemPrompt: "You are Sofia, a friendly HR assistant for Acme Inc. Help employees with vacation requests, leave policies, payroll questions, and HR procedures. Always be empathetic and professional.", model: agentModels[agentIds.hrMain]!, status: "active" },
    { id: agentIds.hrEscalation, workspaceId, teamId: teamHrId, name: "Elena HR Pro", role: "Senior HR Specialist", systemPrompt: "You are Elena, a senior HR specialist. Handle complex HR cases escalated from Sofia. You have authority to approve leave requests up to 5 days.", model: agentModels[agentIds.hrEscalation]!, status: "active" },
    { id: agentIds.itMain, workspaceId, teamId: teamItId, name: "Max IT", role: "IT Support Analyst", systemPrompt: "You are Max, an IT support agent for Acme Inc. Help employees with password resets, software installation, VPN access, and technical troubleshooting.", model: agentModels[agentIds.itMain]!, status: "active" },
    { id: agentIds.itAssets, workspaceId, teamId: teamItId, name: "Asset Bot", role: "Asset Manager", systemPrompt: "You handle hardware asset requests at Acme Inc. Process laptop requests, peripherals, monitors, and phone equipment. Verify budget approval before processing orders over $500.", model: agentModels[agentIds.itAssets]!, status: "active" },
    { id: agentIds.onboardingWelcome, workspaceId, teamId: teamOnboardingId, name: "Alex Welcome", role: "Onboarding Coordinator", systemPrompt: "You are Alex, the first point of contact for new Acme employees. Walk them through day 1 logistics: office access, IT setup, benefits enrollment, and team introductions.", model: agentModels[agentIds.onboardingWelcome]!, status: "active" },
    { id: agentIds.onboardingDocs, workspaceId, teamId: teamOnboardingId, name: "Doc Helper", role: "Documentation Bot", systemPrompt: "You help new Acme employees complete their onboarding paperwork: NDA, direct deposit, benefits forms, and equipment agreements.", model: agentModels[agentIds.onboardingDocs]!, status: "draft" },
  ]).onConflictDoNothing();
