  ...
  const result = await requireAction({
    run: async ({ ctx, tx }) => {
      ...
      const [teams, agents, flows, runs, channels] = await Promise.all([
        tx.select().from(schema.teams).where(eq(schema.teams.workspaceId, wsId)),
        tx.select().from(schema.agents).where(eq(schema.agents.workspaceId, wsId)),
        tx.select().from(schema.flows).where(eq(schema.flows.workspaceId, wsId)),
        tx
          .select()
          ...
          .limit(50),
        tx.select().from(schema.channels).where(eq(schema.channels.workspaceId, wsId)),
      ]);
