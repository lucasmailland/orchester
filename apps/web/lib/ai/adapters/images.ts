...
async function stabilityImages(p: ImageParams, cred: Cred): Promise<ImageResult> {
  ...
  const j = await withRetry(async () => {
    const r = await fetchWithTimeout("https://api.stability.ai/v2beta/stable-image/generate/core", {
      method: "POST",
...
async function ideogramImages(p: ImageParams, cred: Cred): Promise<ImageResult> {
  ...
  form.append("prompt", p.prompt);
  const j = await withRetry(async () => {
    const r = await fetchWithTimeout("https://api.ideogram.ai/v1/ideogram-v3/generate", {
      method: "POST",
...
async function bflImages(p: ImageParams, cred: Cred): Promise<ImageResult> {
  const start = await withRetry(async () => {
    const r = await fetchWithTimeout(`https://api.bfl.ai/v1/${p.model}`, {
      method: "POST",
  ...
  for (let i = 0; i < 60; i++) {
    if (Date.now() > deadline) throw new Error("Black Forest: timeout (deadline de polling excedido).");
    await new Promise((res) => setTimeout(res, 1500));
    const pr = await fetchWithTimeout(pollUrl, { headers: { "x-key": cred.apiKey } }, POLL_TIMEOUT_MS);
...
async function openaiImages(p: ImageParams, cred: Cred, baseURL: string): Promise<ImageResult> {
  ...
  const j = await withRetry(async () => {
    const r = await fetchWithTimeout(`${baseURL}/images/generations`, {
      method: "POST",
...
async function googleImages(p: ImageParams, cred: Cred): Promise<ImageResult> {
  ...
  if (p.model.startsWith("imagen")) {
    const j = await withRetry(async () => {
      const r = await fetchWithTimeout(`${base}/${encodeURIComponent(p.model)}:predict?key=${key}`, {
        method: "POST",
  ...
  const j = await withRetry(async () => {
    const r = await fetchWithTimeout(`${base}/${encodeURIComponent(p.model)}:generateContent?key=${key}`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
