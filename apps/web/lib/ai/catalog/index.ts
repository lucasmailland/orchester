...
export function modelsForConnected(capability: Capability, connectedProviderIds: string[]): ModelDef[] {
...
export function resolveModel(modelId: string): ResolvedModel | null {
  ...
  if (known) {
    ...
    return { provider, capability: known.capability, model: stripPrefix(modelId, known.provider), modelId };
  }
  ...
  if (idx > 0) {
    ...
    if (provider) {
      const cap: Capability = provider.capabilities.includes("chat") ? "chat" : provider.capabilities[0]!;
  ...
  if (legacy) {
    ...
    if (provider) return { provider, capability: "chat", model: modelId, modelId: `${legacy}:${modelId}` };
  }
  return null;
}
