      ...
      properties: {
        nodes: {
          ...
          items: {
            type: "object",
            properties: {
              id: { type: "string", description: "Identificador local del paso (ej. 'inicio', 'agente1')." },
              nodeId: { type: "string", description: "Tipo de paso del catálogo (ej. 'agent', 'http')." },
              label: { type: "string", description: "Nombre claro del paso para una persona." },
              config: { type: "object", description: "Configuración del paso (campos del catálogo)." },
            },
            required: ["id", "nodeId"],
          },
        },
        edges: {
          ...
          items: {
            ...
            properties: {
              ...
              sourceHandle: { type: "string", description: "Para 'Si…entonces' usá 'true'/'false'; para caminos, el valor." },
              label: { type: "string" },
            },
...
export function buildSystemPrompt(locale: Locale = "es"): string {
  return [
    ...
    "llamando a la herramienta set_flow con los pasos y conexiones. Reglas:",
