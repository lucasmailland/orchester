...
export async function assertWithinSpend(workspaceId: string, tx?: WsDb): Promise<void> {
  ...
  try {
    ...
    // COST-6: a cap IS configured (we returned early above if not). We cannot
    // confirm the workspace is under budget, so fail CLOSED on ANY error —
    // a DB hiccup must not silently disable the only hard money cap.
    throw new SpendGuardError(
      "spend cap check failed (database error) — refusing AI call to honor the spend cap"
    );
  }
