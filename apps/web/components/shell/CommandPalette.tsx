  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center bg-black/60 px-4 pt-[15vh] backdrop-blur-sm">
      <Command
        ...
        className="w-full max-w-xl overflow-hidden rounded-xl border border-white/[0.08] bg-zinc-950 shadow-2xl"
      ...
      >
        ...
        <Command.List className="max-h-[400px] overflow-y-auto py-1">
          ...
          <Command.Group heading="Navegación" className="px-1 [&_[cmdk-group-heading]]:px-3 [&_[cmdk-group-heading]]:py-1.5 [&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-wider [&_[cmdk-group-heading]]:text-zinc-500">
            {NAV.map((n) => (
              <Command.Item
                key={n.path}
                onSelect={() => go(n.path)}
                className="flex cursor-pointer items-center gap-3 rounded-md px-3 py-2 text-sm text-zinc-200 aria-selected:bg-violet-500/15 aria-selected:text-violet-200"
              >
                <n.Icon className="h-4 w-4 text-zinc-500" />
                {n.label}
              </Command.Item>
            ))}
          </Command.Group>
          ...
          {items.length > 0 && (
            <Command.Group heading="Recursos" className="px-1 [&_[cmdk-group-heading]]:px-3 [&_[cmdk-group-heading]]:py-1.5 [&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-wider [&_[cmdk-group-heading]]:text-zinc-500">
              {items.map((it) => (
                <Command.Item
                  key={`${it.type}-${it.id}`}
                  onSelect={() =>
