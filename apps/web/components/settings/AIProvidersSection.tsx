    ...
    <div className="space-y-6">
      ...
      <section>
        <h3 className="mb-2 flex items-center gap-2 text-sm font-semibold text-strong">
          <Check className="h-4 w-4 text-emerald-500" /> {t("connectedHeading")}
      ...
      {/* Buscador + filtros */}
      <section>
        <h3 className="mb-2 text-sm font-semibold text-strong">{t("addProviderHeading")}</h3>
...
function ProviderCard({
  ...
  async function save() {
    ...
    setSaving(false);
    if (!r.ok) return setFeedback(t("saveError"));
    setApiKey("");
  ...
  async function test() {
    ...
    setFeedback(
      j.ok
        ? t("testOkFeedback", { count: j.models?.length ?? 0 })
        : t("testErrorFeedback", { error: j.error })
    );
    const all = await fetch("/api/providers").then((x) => x.json());
    onChange(all.find((x: ProviderRow) => x.provider === def.id) ?? null);
  }
