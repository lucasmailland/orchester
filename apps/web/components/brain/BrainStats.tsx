  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-4 text-[10px] text-faint">
        <span>
          Avg confidence: <span className="font-semibold text-body">{(data.totals.avg_confidence * 100).toFixed(0)}%</span>
        </span>
        <span>·</span>
        <span>
          Avg relevance: <span className="font-semibold text-body">{(data.totals.avg_relevance * 100).toFixed(0)}%</span>
        </span>
      </div>
    </div>
  );
