...
export function AgentsPageClient({ agents, teams }: AgentsPageClientProps) {
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, { teamName: string; teamColor: string | null; agents: AgentItem[] }>();
    for (const agent of filtered) {
      const key = agent.teamId ?? "__none__";
      const label = agent.teamName ?? "Sin equipo";
  ...
  const FILTERS: { value: FilterStatus; label: string; count: number }[] = [
    ...
    { value: "active", label: "Activos", count: agents.filter(a => a.status === "active").length },
    { value: "draft", label: "Borrador", count: agents.filter(a => a.status === "draft").length },
    { value: "inactive", label: "Inactivos", count: agents.filter(a => a.status === "inactive").length },
  ];
  ...
  return (
    <div className="space-y-6">
                    ...
                    >
                      ...
                      <div className="p-4 pl-5">
                            ...
                            <div className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-violet-600/15 text-violet-400">
                              ...
                              <span className="absolute -bottom-0.5 -right-0.5">
                                <span className="relative flex h-2.5 w-2.5">
                                  {s.ping && (
                                    <span className={cn("absolute inline-flex h-full w-full animate-ping rounded-full opacity-50", s.dot)} />
                                  )}
                        ...
                        <div className="mt-3 flex items-center justify-between gap-2">
                          ...
                          <span className={cn("rounded-md border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide", s.badge)}>
                            {s.text}
                          </span>
                        </div>
                      </div>
      ...
      {editingAgent && (
        <AgentFormModal
          open
          onClose={() => { setEditingAgent(null); router.refresh(); }}
